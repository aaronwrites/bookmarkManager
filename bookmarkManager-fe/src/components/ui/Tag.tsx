...
const Tag = ({ id, removeTag }: TagProps) => {
	...
	return <div className="relative transition-colors px-3 py-1.5 bg-muted/20 text-muted hover:bg-primary hover:text-white rounded-full group cursor-pointer" onClick={handleClick}>
        <div className="max-w-32 truncate">{data?.tagName}</div>
        { removeTag && <button
