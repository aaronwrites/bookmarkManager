...
const ContentModal = ({ content }: { content: contentType }) => {
	...
	const { mutate, isPending } = useMutation({
		mutationFn: (updatedContent : updateContentType) => updateContent(updatedContent),
		onSuccess: () => {
	...
	const handleSubmit = () => {
		...
		const updatedContent = {
		  ...
		  ...formData,// Convert tags string to an array
		};
	...
	return (
		<div className="h-full w-full grid xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 p-1 gap-5">
			...
			<div className="rounded-xl xl:col-span-1 bg-[#F7F7F7] p-2 flex flex-col gap-5">
				<div className="h-36 bg-white rounded-xl">Tags GO HERE</div>
				...
				<Button className="w-full" disabled={!isModified || isPending} onClick={handleSubmit}>
